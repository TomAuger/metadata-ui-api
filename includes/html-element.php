 ...
 * the $field_args key of 'tags:features[input]:element:size' and that allows the HTML
...
final class WP_Html_Element {
	...
	function get_attributes_html() {
		 ...
		 */
		$valid_attributes = self::get_attributes( $this->tag_name );
	 ...
	 * @return array
	 */
	static function get_attributes( $html_element ) {

		if ( ! ( $attributes = wp_cache_get( $html_element, $group = 'html_element_attributes' ) ) ) {
			...
			wp_cache_set( $html_element, array_fill_keys( $attributes, null ), $group );
